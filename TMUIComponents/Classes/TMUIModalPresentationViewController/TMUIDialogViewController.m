...
+ (void)initialize {
    ...
    dispatch_once(&onceToken, ^{
        TMUIDialogViewController *dialogViewControllerAppearance = TMUIDialogViewController.appearance;
        ...
        dialogViewControllerAppearance.maximumContentViewWidth = [TMUIHelper screenSizeFor55Inch].width - UIEdgeInsetsGetHorizontalValue(dialogViewControllerAppearance.dialogViewMargins);
...
@interface TMUIDialogViewController ()

@property(nonatomic, assign) BOOL hasCustomContentView;
@property(nonatomic,copy) void (^cancelButtonBlock)(TMUIDialogViewController *dialogViewController);
@property(nonatomic,copy) void (^submitButtonBlock)(TMUIDialogViewController *dialogViewController);
@property(nonatomic,strong,readwrite) TMUINavigationTitleView *titleView;
@end

@implementation TMUIDialogViewController
#pragma mark - 生命周期
...
- (void)didInitialize {
    ...
    // 使用自带的TMUINavigationTitleView，支持loading、subTitle
    [self.headerView addSubview:self.titleView];
    
    // 加上分隔线
    _headerViewSeparatorLayer = [CALayer layer];
    [self.headerViewSeparatorLayer tmui_removeDefaultAnimations];
    ...
    [self.footerViewSeparatorLayer tmui_removeDefaultAnimations];
    ...
    [self.buttonSeparatorLayer tmui_removeDefaultAnimations];
    ...
    self.modalPresentationViewController = [[TMUIModalPresentationViewController alloc] init];
...
- (void)viewDidLayoutSubviews {
    ...
    if (isFooterViewShowing) {
        ...
        if (buttonCount == 1) {
            TMUIButton *button = self.cancelButton ? : self.submitButton;
...
- (void)addCancelButtonWithText:(NSString *)buttonText block:(void (^)(__kindof TMUIDialogViewController *))block {
...
- (void)addSubmitButtonWithText:(NSString *)buttonText block:(void (^)(__kindof TMUIDialogViewController *dialogViewController))block {
...
- (TMUIButton *)generateButtonWithText:(NSString *)buttonText {
...
- (void)handleCancelButtonEvent:(TMUIButton *)cancelButton {
...
- (void)handleSubmitButtonEvent:(TMUIButton *)submitButton {
...
#pragma mark - <TMUIModalPresentationComponentProtocol>
...
@implementation TMUIDialogSelectionViewController (UIAppearance)
...
+ (void)initialize {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        TMUIDialogSelectionViewController.appearance.rowHeight = TableViewCellNormalHeight;
    });
}

@end

const NSInteger TMUIDialogSelectionViewControllerSelectedItemIndexNone = -1;

@interface TMUIDialogSelectionViewController ()

@property(nonatomic,strong,readwrite) TMUITableView *tableView;
@end

@implementation TMUIDialogSelectionViewController

- (void)didInitialize {
    [super didInitialize];
    
    self.selectedItemIndex = TMUIDialogSelectionViewControllerSelectedItemIndexNone;
    self.selectedItemIndexes = [[NSMutableSet alloc] init];
    
    self.tableView = [[TMUITableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
    ...
    self.tableView.backgroundColor = self.contentViewBackgroundColor;// TMUIDialogSelectionViewController 使用了 customContentView，所以默认不会自动应用到 self.contentViewBackgroundColor，这里手动应用一次
}

- (void)viewDidAppear:(BOOL)animated {
    [super viewDidAppear:animated];
    // 当前的分组不在可视区域内，则滚动到可视区域（只对单选有效）
    if (self.selectedItemIndex != TMUIDialogSelectionViewControllerSelectedItemIndexNone && self.selectedItemIndex < self.items.count && ![self.tableView tmui_cellVisibleAtIndexPath:[NSIndexPath indexPathForRow:self.selectedItemIndex inSection:0]]) {
...
#pragma mark - <TMUITableViewDataSource, TMUITableViewDelegate>
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.allowsMultipleSelection) {
        ...
        if ([tableView tmui_cellVisibleAtIndexPath:indexPath]) {
        ...
        if (self.selectedItemIndex != TMUIDialogSelectionViewControllerSelectedItemIndexNone) {
            ...
            isSelectedIndexPathBeforeVisible = [tableView tmui_cellVisibleAtIndexPath:selectedIndexPathBefore];
        }
...
@implementation TMUIDialogTextFieldViewController (UIAppearance)
...
+ (void)initialize {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        TMUIDialogTextFieldViewController *dialogTextFieldViewControllerAppearance = TMUIDialogTextFieldViewController.appearance;
...
@interface TMUIDialogTextFieldViewController ()<TMUITextFieldDelegate>

@property(nonatomic, strong) UIScrollView *scrollView;
@property(nonatomic, strong) NSMutableArray<TMUILabel *> *mutableTitleLabels;
@property(nonatomic, strong) NSMutableArray<TMUITextField *> *mutableTextFields;
@property(nonatomic, strong) NSMutableArray<CALayer *> *mutableSeparatorLayers;
@end

@implementation TMUIDialogTextFieldViewController
...
- (void)addTextFieldWithTitle:(NSString *)textFieldTitle configurationHandler:(void (^)(TMUILabel *titleLabel, TMUITextField *textField, CALayer *separatorLayer))configurationHandler {
    TMUILabel *label = [self generateTextFieldTitleLabel];
    ...
    TMUITextField *textField = [self generateTextField];
...
- (TMUILabel *)generateTextFieldTitleLabel {
    TMUILabel *textFieldLabel = [[TMUILabel alloc] init];
...
- (TMUITextField *)generateTextField {
...
- (CALayer *)generateTextFieldSeparatorLayer {
    CALayer *textFieldSeparatorLayer = [self tmui_separatorLayer];
...
- (void)viewDidLayoutSubviews {
    ...
    NSAssert(self.mutableTitleLabels.count == self.mutableTextFields.count && self.mutableTextFields.count == self.mutableSeparatorLayers.count, NSStringFromClass(self.class), @"标题、输入框、分隔线的数量不匹配");
    
    CGFloat minY = 0;
    
    for (NSInteger i = 0; i < self.mutableTitleLabels.count; i++) {
        TMUILabel *label = self.mutableTitleLabels[i];
        TMUITextField *textField = self.mutableTextFields[i];
...
- (void)setTextFieldLabelFont:(UIFont *)textFieldLabelFont {
    ...
    [self.mutableTitleLabels enumerateObjectsUsingBlock:^(TMUILabel * _Nonnull label, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setTextFieldFont:(UIFont *)textFieldFont {
    ...
    [self.mutableTextFields enumerateObjectsUsingBlock:^(TMUITextField * _Nonnull textField, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSArray<TMUILabel *> *)textFieldTitleLabels {
    return self.mutableTitleLabels.copy;
}

- (NSArray<TMUITextField *> *)textFields {
...
- (BOOL)shouldEnabledSubmitButton {
    ...
    if (self.enablesSubmitButtonAutomatically) {
        ...
        [self.mutableTextFields enumerateObjectsUsingBlock:^(TMUITextField * _Nonnull textField, NSUInteger idx, BOOL * _Nonnull stop) {
            NSInteger textLength = textField.text.tmui_trim.length;
...
- (void)handleTextFieldTextDidChangeEvent:(TMUITextField *)textField {
...
- (CGSize)preferredContentSizeInModalPresentationViewController:(TMUIModalPresentationViewController *)controller keyboardHeight:(CGFloat)keyboardHeight limitSize:(CGSize)limitSize {
    ...
    for (TMUILabel *label in self.mutableTitleLabels) {
        if (!label.hidden) {
...
- (BOOL)textFieldShouldReturn:(TMUITextField *)textField {
